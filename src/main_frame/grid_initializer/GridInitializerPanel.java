...
public class GridInitializerPanel extends JPanel {
	...
	public GridInitializerPanel(Graph graph, GridConfiguration gridConfiguration, ArrayList<Color> colors, ArrayList<Rule> rules) {
		...
		btnColorRandom.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				synchronized(GridInitializerPanel.this.grid) { //ci sincronizziamo con la griglia perch� ci sono altri modi in cui potrebbe essere modificata
				...
				if(btnFindConfiguration.isEnabled()) {
					...
					optimizer = new Optimizer(new ArrayList<>(colors), copyRules, graph.copy(), opK, opCycLen, opSizeX, opSizeY) {
						...
						public void evolutionCompleted() {
							synchronized(GridInitializerPanel.this.grid) { //ci sincronizziamo con la griglia perch� sono altri modi in cui potrebbe essere modificata
								if(GridInitializerPanel.this.graph != null) super.getBestGene().setGraph(GridInitializerPanel.this.graph); //quando l'evoluzione � completa settiamo il grafo
